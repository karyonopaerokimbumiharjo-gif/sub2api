export default {
    ...
    proxies: {
      ...
      clashImportTitle: '导入 Clash 节点',
      clashImportHint: '上传 Clash 配置，或直接粘贴配置内容/代理 URL。导入前会预览节点并去重。',
      clashImportSupported: '当前可直接接入：HTTP、HTTPS、SOCKS5/SOCKS5H。SS、VMess、VLESS、Trojan 等节点会显示为不支持，不会被静默导入。',
      clashImportFile: 'Clash 配置文件',
      clashImportSelectFile: '请选择 YAML、JSON 或文本文件',
      clashImportPastePlaceholder: '粘贴 Clash YAML/JSON，或每行一个 http://、socks5:// 代理 URL',
      clashImportValid: '可导入 {count} 个',
      clashImportUnsupported: '不支持 {count} 个',
      clashImportInvalid: '无效 {count} 个',
      clashImporting: '导入中...',
      clashImportButton: '导入节点',
      clashImportNoValid: '没有可导入的节点',
      clashImportSuccess: 'Clash 导入完成：新增 {created} 个，跳过 {skipped} 个',
      clashImportFailed: 'Clash 导入失败',
      clashImportCopyTemplate: '复制模板',
      clashImportDownloadTemplate: '下载模板',
      clashImportTemplateCopied: '模板已复制',
      clashImportTemplateCopyFailed: '无法复制模板',
      ...
      sub2AccountsCount: 'Sub2 账号配置 {count}',
      testExitLabel: '代理测试出口',
      egressEvidenceHint: '此页 IP 和地区来自该代理的连接测试。绑定数表示保存的配置，不代表某个 Pi / CPA 账号当前请求的实际出口。',
      savedBindingHint: '包含已停用或未用于当前业务的 CPA 凭证绑定；不能据此判断 Pi 出口。',
      cpaCredentialsCount: '已保存 CPA 绑定 {count}',
      cpaBindingsUnknown: 'CPA 绑定状态未知',
      accountsTitle: '已保存此代理配置的账号',
      accountsEmpty: '暂无 Sub2 账号保存此代理配置',
