export default {
  ...
  usage: {
    ...
    latencyDuration: '总耗时',
    latencyFirstTokenWithoutAudit: '首字（不含审）',
    latencyFirstTokenWithAudit: '首字（含审）',
    latencyDurationWithoutAudit: '总耗时（不含审）',
    latencyDurationWithAudit: '总耗时（含审）',
    audit: '审核结果', auditEvidenceHint: '仅显示实际留存的审核证据；缺少历史记录不代表未审核。原生审核记录按留存策略过期后，结果将不再显示。', auditSource: { native: '原生审计', custom: '自定义审计' }, auditResult: { partial: '部分审计，未覆盖全文', passed: '通过', blocked: '已拦截', flagged: '风险标记', error: '审核异常', inconclusive: '未得结论', notAudited: '未审核', timed: '已计时，结果未留存', unknown: '无留存记录，状态未知' },
    promptAuditLatency: '提示词审计耗时',
    tps: '平均 TPS',
    tpsHint: '平均输出速率 = 输出 token 数 ÷ 记录的总耗时（包含首字等待），所有请求类型使用同一口径。这是整次请求的平均值，不代表模型解码速度。缺少有效 token 或耗时数据、图片请求显示 —。',
