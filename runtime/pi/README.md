...
For a first request, explicit JSON `null` values for `previous_response_id`, `max_output_tokens` and `client_metadata` are treated as omitted fields. Non-null `max_output_tokens` is accepted and forwarded with the Responses request. The gateway removes `client_metadata` and `previous_response_id` before the private runtime, so a client can reuse the same public request format when switching between CPA and Pi.
