...
const replayModel={id:model,name:model,provider:'openai-codex',api:'openai-codex-responses',baseUrl:'https://chatgpt.com/backend-api',reasoning:true,input:['text','image'],cost:{input:0,output:0,cacheRead:0,cacheWrite:0},contextWindow:128000,maxTokens:4096};
...
let passed=true,stage='setup';
async function run(turn,request){
 stage=`turn_${turn}_stream`;
 ...
 stage=`turn_${turn}_replay`;
 return {...response,replayItems:convertResponsesMessages(replayModel,{messages:[result.result]},new Set(['openai','openai-codex','opencode']),{includeSystemPrompt:false}),output:response.output?.length?response.output:output};
}
try{
 stage='first_turn';
...
}catch(error){const reasons=['native_protocol_failed','terminal_response_missing','tool_call_mismatch','tool_result_mismatch','verification_response_limit'];console.log(JSON.stringify({result:'FAIL',stage,reason:reasons.includes(error.message)?error.message:'native_acceptance_failed',errorType:error?.name||'Error'}));process.exitCode=1}
