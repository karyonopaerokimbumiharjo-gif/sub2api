...
type UsageLog struct {
	...
	// Handler entry to first upstream dispatch, including auth/audit/queue
	VpsLatencyMs *int `json:"vps_latency_ms,omitempty"`
	// Synchronous prompt-audit wall-clock delay before upstream dispatch
	PromptAuditLatencyMs *int `json:"prompt_audit_latency_ms,omitempty"`
...
func (*UsageLog) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case usagelog.FieldID, usagelog.FieldUserID, usagelog.FieldAPIKeyID, usagelog.FieldAccountID, usagelog.FieldChannelID, usagelog.FieldGroupID, usagelog.FieldSubscriptionID, usagelog.FieldInputTokens, usagelog.FieldOutputTokens, usagelog.FieldCacheCreationTokens, usagelog.FieldCacheReadTokens, usagelog.FieldCacheCreation5mTokens, usagelog.FieldCacheCreation1hTokens, usagelog.FieldBillingType, usagelog.FieldDurationMs, usagelog.FieldFirstTokenMs, usagelog.FieldVpsLatencyMs, usagelog.FieldPromptAuditLatencyMs, usagelog.FieldImageCount, usagelog.FieldVideoCount, usagelog.FieldVideoDurationSeconds:
...
func (_m *UsageLog) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case usagelog.FieldVpsLatencyMs:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field vps_latency_ms", values[i])
			} else if value.Valid {
				_m.VpsLatencyMs = new(int)
				*_m.VpsLatencyMs = int(value.Int64)
			}
		case usagelog.FieldPromptAuditLatencyMs:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field prompt_audit_latency_ms", values[i])
			} else if value.Valid {
				_m.PromptAuditLatencyMs = new(int)
				*_m.PromptAuditLatencyMs = int(value.Int64)
			}
...
func (_m *UsageLog) String() string {
	...
	builder.WriteString(", ")
	if v := _m.VpsLatencyMs; v != nil {
		builder.WriteString("vps_latency_ms=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
	if v := _m.PromptAuditLatencyMs; v != nil {
		builder.WriteString("prompt_audit_latency_ms=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
