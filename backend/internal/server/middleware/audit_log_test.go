...
func TestPiAuthImportRouteOmitsEntireAuditBody(t *testing.T) {
	gin.SetMode(gin.TestMode)
	require.Contains(t, auditBodyOmittedRoutes, "POST /api/v1/admin/openai/import-pi-auth")
	...
	router.Use(gin.HandlerFunc(NewAuditLogMiddleware(auditService)))
	router.POST("/api/v1/admin/openai/import-pi-auth", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"ok": true})
	})
	request := httptest.NewRequest(http.MethodPost, "/api/v1/admin/openai/import-pi-auth",
		bytes.NewBufferString(`{"content":"{\"tokens\":{\"refresh_token\":\"audit-pi-canary\"}}","pi_owner_user_id":7,"group_ids":[42]}`))
	request.Header.Set("Content-Type", "application/json")
	recorder := httptest.NewRecorder()
	router.ServeHTTP(recorder, request)
	require.Equal(t, http.StatusOK, recorder.Code)
	...
	require.Len(t, logs, 1)
	require.Equal(t, "<credential-bearing body omitted>", logs[0].RequestBody)
	require.NotContains(t, logs[0].RequestBody, "audit-pi-canary")
}
