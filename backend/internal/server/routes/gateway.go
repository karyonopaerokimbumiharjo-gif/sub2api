...
func RegisterGatewayRoutes(
	...
	// Local bridge control does not enter the model or prompt-capture pipeline.
	r.POST("/v1/j/bridge/:action", middleware.RequestBodyLimit(3<<20), gin.HandlerFunc(apiKeyAuth), h.OpenAIGateway.JBridge)
	bodyLimit := middleware.RequestBodyLimit(cfg.Gateway.MaxBodySize)
	textBodyLimit := middleware.RequestBodyLimit(cfg.Gateway.TextMaxBodySize)
	clientRequestID := middleware.ClientRequestID()
	opsErrorLogger := handler.OpsErrorLoggerMiddleware(opsService)
	outputFinalizer := handler.SecurityAuditOutputFinalizer()
	...
	gateway.Use(opsErrorLogger, outputFinalizer)
	gateway.Use(endpointNorm)
	gateway.Use(gin.HandlerFunc(apiKeyAuth), h.OpenAIGateway.JModeContext)
	...
	gemini.Use(opsErrorLogger, outputFinalizer)
	...
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, outputFinalizer, endpointNorm, gin.HandlerFunc(apiKeyAuth), h.OpenAIGateway.JModeContext, groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)
	}
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, outputFinalizer, endpointNorm, gin.HandlerFunc(apiKeyAuth), h.OpenAIGateway.JModeContext, groupModelAllowlist, compositeTarget, requireGroupAnthropic)
	{
	...
	antigravityV1.Use(opsErrorLogger, outputFinalizer)
	...
	antigravityV1Beta.Use(opsErrorLogger, outputFinalizer)
