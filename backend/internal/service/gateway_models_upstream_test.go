...
func TestGetAvailableModels_DoesNotSynthesizeRetiredAliases(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{StatusCode: http.StatusOK,
		Body: io.NopCloser(strings.NewReader(`{"data":[{"id":"gpt-5.6-luna"}]}`))}}
	repo := &modelsListAccountRepoStub{byGroup: map[int64][]Account{groupID: {
		{ID: 32, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Credentials: map[string]any{
			"api_key": "test-key", "base_url": "http://cpa:8317/v1",
		}},
	}}}
	svc := &GatewayService{accountRepo: repo, httpUpstream: upstream, cfg: cfg}
	require.Equal(t, []string{"gpt-5.6-luna"}, svc.GetAvailableModels(context.Background(), &groupID, PlatformOpenAI))
}
