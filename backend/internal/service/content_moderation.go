...
func (s *ContentModerationService) UpdateConfig(ctx context.Context, input UpdateContentModerationConfigInput) (*ContentModerationConfigView, error) {
	...
	if err := s.validateConfig(ctx, cfg.effectiveEngine(cfg.Engine)); err != nil {
		return nil, err
	}
	cfg.normalize()
...
func (s *ContentModerationService) moderationHTTPClient(ctx context.Context, cfg *ContentModerationConfig) (*http.Client, error) {
	...
	if cpapolicy.ValidateBaseURL(cfg.BaseURL) == nil {
		if cfg.ProxyID != nil {
	...
	if client == nil {
		client = cpaModerationClient
	}
	isolated := *client
	isolated.CheckRedirect = cpapolicy.NoRedirect
	return &isolated, nil
}
...
// Native audit endpoints are separate from the inference backend boundary.
func validateNativeModerationDestination(cfg *ContentModerationConfig) error {
	if cfg == nil {
		return cpapolicy.Required()
	}
	u, err := url.Parse(strings.TrimSpace(cfg.BaseURL))
	if err == nil && u.Scheme == "https" && u.User == nil && u.RawQuery == "" && u.Fragment == "" && u.RawPath == "" && (strings.TrimRight(u.Path, "/") == "" || strings.TrimRight(u.Path, "/") == "/v1") {
		if (cfg.Engine == ContentModerationEngineTypeSafe && strings.EqualFold(u.Host, "api.typesafe.ai")) || (cfg.Engine == ContentModerationEngineOpenAI && strings.EqualFold(u.Host, "api.openai.com")) {
			return nil
		}
	}
	if cfg.Engine == ContentModerationEngineTypeSafe {
		return infraerrors.BadRequest("INVALID_CONTENT_MODERATION_BASE_URL", "TypeSafe 审计仅允许官方 HTTPS 地址")
	}
	return cpapolicy.ValidateBaseURL(cfg.BaseURL)
}

func (s *ContentModerationService) NativeAuditReady(ctx context.Context) bool {
	if s == nil || s.settingRepo == nil {
		return false
	}
	cfg, err := s.loadConfig(ctx)
