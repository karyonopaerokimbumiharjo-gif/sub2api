...
func TestContentModerationEngineProfilesPreserveLegacy(t *testing.T) {
	repo := &contentModerationTestSettingRepo{values: map[string]string{SettingKeyContentModerationConfig: `{"enabled":true,"mode":"pre_block","api_key":"old-openai-key","base_url":"https://api.openai.com","model":"omni","thresholds":{"sexual":0.65},"auto_ban_enabled":true}`}}
	...
	require.False(t, view.AutoBanEnabled) // Existing deployment never permanently bans from a model-only result.
	...
	require.Equal(t, "https://api.openai.com", stored.BaseURL)
...
func TestContentModerationTypeSafeAllCategoriesAndImages(t *testing.T) {
	...
	s := &ContentModerationService{httpClient: nativeAuditTestClient(server)}
...
func TestContentModerationTypeSafeFailureOpenAndKeyIsolation(t *testing.T) {
	...
	s := &ContentModerationService{httpClient: nativeAuditTestClient(server), repo: repo}
	cfg := defaultContentModerationConfig()
	cfg.Engine = "typesafe"
	cfg.BaseURL = "https://api.typesafe.ai"
...
func TestContentModerationTypeSafeTestDoesNotSwitchActiveEngine(t *testing.T) {
	...
	cfg.TypeSafe.BaseURL = "https://api.typesafe.ai"
	raw, err := json.Marshal(cfg)
	require.NoError(t, err)
	repo.values[SettingKeyContentModerationConfig] = string(raw)
	s := &ContentModerationService{settingRepo: repo, httpClient: nativeAuditTestClient(server)}
...
type nativeAuditRoundTripper func(*http.Request) (*http.Response, error)

func (f nativeAuditRoundTripper) RoundTrip(r *http.Request) (*http.Response, error) { return f(r) }
func nativeAuditTestClient(server *httptest.Server) *http.Client {
	client := server.Client()
	transport := client.Transport
	if transport == nil {
		transport = http.DefaultTransport
	}
	target, _ := http.NewRequest(http.MethodGet, server.URL, nil)
	client.Transport = nativeAuditRoundTripper(func(r *http.Request) (*http.Response, error) {
		cloned := r.Clone(r.Context())
		u := *r.URL
		cloned.URL = &u
		cloned.URL.Scheme = target.URL.Scheme
		cloned.URL.Host = target.URL.Host
		cloned.Host = target.URL.Host
