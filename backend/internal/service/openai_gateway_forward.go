...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	body = s.applySafetyIdentifier(c, account, body)
	beginUpstreamResponseModelObservation(c)
	if account.UsesNativePiRuntime() {
		return s.forwardNativePi(ctx, c, account, body)
	}
	...
	for {
		...
		// A non-streaming response has no usage stream to drain after the
		// downstream client leaves. Keep its upstream bound to the request so
		// a timed-out client does not keep consuming the account slot and quota.
		// Streaming requests remain detached to finish usage accounting.
