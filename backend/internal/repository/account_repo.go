...
func createAccountRecord(ctx context.Context, client *dbent.Client, account *service.Account) error {
	...
	if err := service.ValidateExecutionAccount(account); err != nil {
		return err
	}
...
func lockAndMergeAccountProbeExtra(
	...
	extra := service.StripLegacyCodexTicketExtra(copyJSONMap(normalizeJSONMap(account.Extra)))
...
func (r *accountRepository) BulkUpdate(ctx context.Context, ids []int64, updates service.AccountBulkUpdate) (int64, error) {
	...
	if len(updates.Credentials) > 0 || updates.ProxyID != nil {
		...
		for _, account := range accounts {
			...
			if err := service.ValidateExecutionAccount(&candidate); err != nil {
				return 0, err
			}
		}
	}
