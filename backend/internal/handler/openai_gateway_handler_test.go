...
func TestOpenAIResponsesWebSocket_ContentModerationBlocksFirstFrame(t *testing.T) {
	...
	// Exercise the production destination check with the approved native origin.
	// The constructor captures this test-only transport; restore the global
	// immediately so the WebSocket fixture and other requests are unaffected.
	moderationSvc := func() *service.ContentModerationService {
		original := http.DefaultTransport
		defer func() { http.DefaultTransport = original }()
		http.DefaultTransport = moderationHandlerRoundTripper(func(req *http.Request) (*http.Response, error) {
			if req.URL.Scheme != "https" || req.URL.Host != "api.openai.com" {
				return nil, fmt.Errorf("unexpected moderation test destination")
			}
			copy := req.Clone(req.Context())
			u := *req.URL
			copy.URL = &u
			copy.URL.Scheme, copy.URL.Host = "http", moderationServer.Listener.Addr().String()
...
type moderationHandlerRoundTripper func(*http.Request) (*http.Response, error)

func (f moderationHandlerRoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	return f(req)
}
